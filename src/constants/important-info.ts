import type { Language } from '@/constants/i18n';
import { getMedicalDisclaimerText, isLegalSectionTitle } from '@/constants/legal-info';

const IMPORTANT_INFO_RU = `Тревожное расстройство — это состояние, которое поддерживается сразу несколькими взаимосвязанными факторами. Изменения даже в одном из них постепенно помогают ослабить тревогу.

🧠 Особенности работы нервной системы
Нервная система человека с тревожным расстройством становится более чувствительной к сигналам опасности. Она может запускать реакцию тревоги даже тогда, когда реальной угрозы нет.
Примеры:
недостаток сна;
длительный стресс;
переутомление;
кофеин или другие стимуляторы;
пропуск назначенных лекарств;
восстановление после панической атаки, когда нервная система еще остается "настороженной".
Именно поэтому в дневнике важно отмечать: сон, прием лекарств, физическую активность и общее самочувствие.

💭 Мыслительные привычки
При тревожном расстройстве мозг начинает автоматически искать опасность и часто переоценивает вероятность плохого исхода.
Примеры:
«У меня колет в груди — наверное, это сердце».
«Если закружилась голова, я потеряю сознание».
«Если тревога появилась сегодня, значит завтра будет еще хуже».
«Я никогда не смогу жить нормально».
Эти мысли возникают автоматически и кажутся убедительными, хотя чаще всего не соответствуют действительности.
Поэтому полезно записывать события дня и свои переживания, чтобы позже увидеть ситуацию более объективно.

🚶 Поведение
Когда человек начинает избегать ситуаций, которые кажутся опасными, тревога временно уменьшается. Но именно это избегание закрепляет тревожное расстройство.
Примеры:
перестать ездить в метро;
отказаться от походов в магазин;
постоянно измерять давление или пульс;
носить с собой лекарства «на всякий случай» и чувствовать себя без них небезопасно;
избегать физических нагрузок из-за страха учащенного сердцебиения.
Такое поведение приносит кратковременное облегчение, но не позволяет мозгу убедиться, что ситуация безопасна.

❤️ Физиологические реакции организма
Тревога всегда сопровождается изменениями в теле. Они являются нормальной частью реакции организма на стресс, но могут восприниматься как признаки опасной болезни.
Примеры:
учащенное сердцебиение;
нехватка воздуха;
напряжение мышц;
дрожь;
потливость;
головокружение;
ощущение кома в горле;
дискомфорт в желудке;
покалывание в руках или лице.
Эти симптомы неприятны, но сами по себе обычно не представляют опасности.

И самое главное
Эти четыре фактора постоянно влияют друг на друга.
Например:
Вы плохо спали → нервная система стала более чувствительной → утром почувствовали учащенное сердцебиение → возникла мысль «со мной что-то не так» → решили отменить тренировку и остаться дома → тревога уменьшилась на несколько часов, но мозг «запомнил», что избегание помогает → в следующий раз тревога возникнет еще быстрее.
Именно поэтому дневник включает сразу несколько разделов: сон, лекарства, физическую активность, триггеры, события дня и уровень тревоги. Вместе они помогают увидеть не отдельные симптомы, а всю цепочку событий. Это делает состояние более понятным, а лечение — более осознанным и эффективным. Надо понять, что тревога — это не случайный хаос, а процесс, который можно постепенно научиться распознавать и менять.`;

const IMPORTANT_INFO_EN = `Anxiety disorder is maintained by several interconnected factors. Changing even one of them gradually helps reduce anxiety.

🧠 How the nervous system works
In anxiety disorder, the nervous system becomes more sensitive to danger signals. It may trigger anxiety even when there is no real threat.
Examples:
lack of sleep;
prolonged stress;
burnout;
caffeine or other stimulants;
missing prescribed medication;
recovery after a panic attack, when the nervous system is still on high alert.
That is why the diary tracks sleep, medication, physical activity, and overall well-being.

💭 Thinking habits
With anxiety disorder, the brain automatically looks for danger and often overestimates the likelihood of a bad outcome.
Examples:
"My chest hurts — it must be my heart."
"If I feel dizzy, I will faint."
"If I felt anxious today, tomorrow will be worse."
"I will never be able to live normally."
These thoughts arise automatically and feel convincing, even though they often do not match reality.
Writing down daily events and feelings helps you see situations more objectively later.

🚶 Behavior
When a person starts avoiding situations that feel dangerous, anxiety temporarily decreases. But this avoidance reinforces anxiety disorder.
Examples:
stopping subway travel;
avoiding grocery stores;
constantly checking blood pressure or pulse;
carrying medication "just in case" and feeling unsafe without it;
avoiding exercise because of fear of a racing heart.
This behavior brings short-term relief but does not let the brain learn that the situation is safe.

❤️ Physical reactions
Anxiety always comes with bodily changes. They are a normal part of the stress response but may feel like signs of a dangerous illness.
Examples:
racing heartbeat;
shortness of breath;
muscle tension;
trembling;
sweating;
dizziness;
lump in the throat;
stomach discomfort;
tingling in the hands or face.
These symptoms are unpleasant but are usually not dangerous on their own.

Most importantly
These four factors constantly influence one another.
For example:
You slept poorly → your nervous system became more sensitive → you noticed a racing heart in the morning → the thought appeared that "something is wrong with me" → you canceled exercise and stayed home → anxiety decreased for a few hours, but the brain "learned" that avoidance helps → next time anxiety appears even faster.
That is why the diary includes sleep, medication, physical activity, triggers, daily events, and anxiety level. Together they help you see not isolated symptoms but the whole chain of events. This makes your state easier to understand and treatment more conscious and effective. Anxiety is not random chaos — it is a process you can gradually learn to recognize and change.`;

const IMPORTANT_INFO_ES = `El trastorno de ansiedad se mantiene por varios factores interconectados. Cambiar incluso uno de ellos ayuda gradualmente a reducir la ansiedad.

🧠 Cómo funciona el sistema nervioso
Con el trastorno de ansiedad, el sistema nervioso se vuelve más sensible a las señales de peligro. Puede activar la ansiedad incluso cuando no hay una amenaza real.
Ejemplos:
falta de sueño;
estrés prolongado;
agotamiento;
cafeína u otros estimulantes;
omitir la medicación recetada;
recuperación tras un ataque de pánico, cuando el sistema nervioso sigue en alerta.
Por eso el diario registra el sueño, la medicación, la actividad física y el bienestar general.

💭 Hábitos de pensamiento
Con el trastorno de ansiedad, el cerebro busca peligro automáticamente y a menudo sobreestima la probabilidad de un mal resultado.
Ejemplos:
«Me duele el pecho — debe ser el corazón».
«Si me mareo, me desmayaré».
«Si hoy tuve ansiedad, mañana será peor».
«Nunca podré vivir con normalidad».
Estos pensamientos surgen solos y parecen convincentes, aunque a menudo no se corresponden con la realidad.
Anotar los eventos del día y las emociones ayuda a ver la situación con más objetividad después.

🚶 Comportamiento
Cuando una persona empieza a evitar situaciones que le parecen peligrosas, la ansiedad disminuye temporalmente. Pero esa evitación refuerza el trastorno de ansiedad.
Ejemplos:
dejar de usar el metro;
evitar ir a la tienda;
medir constantemente la presión o el pulso;
llevar medicación «por si acaso» y sentirse inseguro sin ella;
evitar el ejercicio por miedo al taquicardia.
Este comportamiento alivia a corto plazo, pero no permite que el cerebro aprenda que la situación es segura.

❤️ Reacciones físicas
La ansiedad siempre va acompañada de cambios en el cuerpo. Son una parte normal de la respuesta al estrés, pero pueden sentirse como signos de una enfermedad grave.
Ejemplos:
taquicardia;
falta de aire;
tensión muscular;
temblor;
sudoración;
mareo;
nudo en la garganta;
malestar estomacal;
hormigueo en manos o cara.
Estos síntomas son desagradables, pero por sí solos suelen no ser peligrosos.

Lo más importante
Estos cuatro factores se influyen constantemente entre sí.
Por ejemplo:
Durmió mal → el sistema nervioso se volvió más sensible → por la mañana notó taquicardia → apareció el pensamiento «algo me pasa» → canceló el entrenamiento y se quedó en casa → la ansiedad disminuyó unas horas, pero el cerebro «aprendió» que evitar ayuda → la próxima vez la ansiedad aparece aún más rápido.
Por eso el diario incluye sueño, medicación, actividad física, desencadenantes, eventos del día y nivel de ansiedad. Juntos ayudan a ver no síntomas aislados, sino toda la cadena de eventos. Esto hace el estado más comprensible y el tratamiento más consciente y eficaz. La ansiedad no es caos aleatorio — es un proceso que puede aprender a reconocer y cambiar gradualmente.`;

const IMPORTANT_INFO_FR = `Le trouble anxieux est maintenu par plusieurs facteurs interconnectés. Modifier ne serait-ce qu'un seul d'entre eux aide progressivement à réduire l'anxiété.

🧠 Fonctionnement du système nerveux
Avec un trouble anxieux, le système nerveux devient plus sensible aux signaux de danger. Il peut déclencher l'anxiété même en l'absence de menace réelle.
Exemples :
manque de sommeil ;
stress prolongé ;
épuisement ;
caféine ou autres stimulants ;
oubli d'un médicament prescrit ;
convalescence après une crise de panique, lorsque le système nerveux reste en alerte.
C'est pourquoi le journal suit le sommeil, les médicaments, l'activité physique et le bien-être général.

💭 Habitudes de pensée
Avec un trouble anxieux, le cerveau cherche automatiquement le danger et surestime souvent la probabilité d'un mauvais résultat.
Exemples :
« J'ai mal à la poitrine — ce doit être le cœur. »
« Si j'ai la tête qui tourne, je vais m'évanouir. »
« Si j'ai eu de l'anxiété aujourd'hui, demain sera pire. »
« Je ne pourrai jamais vivre normalement. »
Ces pensées surgissent automatiquement et semblent convaincantes, bien qu'elles ne correspondent souvent pas à la réalité.
Noter les événements de la journée et ses émotions aide à voir la situation plus objectivement par la suite.

🚶 Comportement
Lorsqu'une personne commence à éviter des situations qui lui semblent dangereuses, l'anxiété diminue temporairement. Mais cette évitement renforce le trouble anxieux.
Exemples :
arrêter de prendre le métro ;
éviter les courses ;
mesurer constamment la tension ou le pouls ;
porter des médicaments « au cas où » et se sentir en insécurité sans eux ;
éviter l'exercice par peur d'une accélération du cœur.
Ce comportement apporte un soulagement à court terme, mais empêche le cerveau d'apprendre que la situation est sûre.

❤️ Réactions physiques
L'anxiété s'accompagne toujours de changements corporels. Ils font partie normale de la réponse au stress, mais peuvent être ressentis comme des signes d'une maladie grave.
Exemples :
accélération du cœur ;
essoufflement ;
tension musculaire ;
tremblements ;
transpiration ;
vertiges ;
boule dans la gorge ;
inconfort gastrique ;
fourmillements dans les mains ou le visage.
Ces symptômes sont désagréables, mais ne sont généralement pas dangereux en eux-mêmes.

L'essentiel
Ces quatre facteurs s'influencent constamment les uns les autres.
Par exemple :
Vous avez mal dormi → le système nerveux est devenu plus sensible → le matin, vous avez remarqué une accélération du cœur → la pensée « quelque chose ne va pas » est apparue → vous avez annulé l'entraînement et resté à la maison → l'anxiété a diminué pendant quelques heures, mais le cerveau a « appris » que l'évitement aide → la prochaine fois, l'anxiété apparaît encore plus vite.
C'est pourquoi le journal comprend le sommeil, les médicaments, l'activité physique, les déclencheurs, les événements de la journée et le niveau d'anxiété. Ensemble, ils aident à voir non pas des symptômes isolés, mais toute la chaîne d'événements. Cela rend l'état plus compréhensible et le traitement plus conscient et efficace. L'anxiété n'est pas un chaos aléatoire — c'est un processus que l'on peut apprendre à reconnaître et à modifier progressivement.`;

const IMPORTANT_INFO_DE = `Eine Angststörung wird durch mehrere miteinander verbundene Faktoren aufrechterhalten. Schon eine Veränderung an einem davon hilft, Angst allmählich zu verringern.

🧠 Wie das Nervensystem arbeitet
Bei einer Angststörung reagiert das Nervensystem empfindlicher auf Gefahrensignale. Es kann Angst auslösen, auch wenn keine reale Bedrohung besteht.
Beispiele:
Schlafmangel;
anhaltender Stress;
Übermüdung;
Koffein oder andere Stimulanzien;
vergessene verschriebene Medikamente;
Erholung nach einer Panikattacke, wenn das Nervensystem noch in Alarmbereitschaft ist.
Deshalb erfasst das Tagebuch Schlaf, Medikamente, körperliche Aktivität und allgemeines Befinden.

💭 Denkgewohnheiten
Bei einer Angststörung sucht das Gehirn automatisch nach Gefahr und überschätzt oft die Wahrscheinlichkeit eines schlechten Ausgangs.
Beispiele:
„Meine Brust tut weh — das muss das Herz sein.“
„Wenn mir schwindelig wird, werde ich ohnmächtig.“
„Wenn ich heute Angst hatte, wird es morgen schlimmer.“
„Ich werde nie normal leben können.“
Diese Gedanken entstehen automatisch und wirken überzeugend, obwohl sie oft nicht der Realität entsprechen.
Tägliche Ereignisse und Gefühle aufzuschreiben hilft, die Situation später objektiver zu sehen.

🚶 Verhalten
Wenn jemand Situationen meidet, die gefährlich erscheinen, sinkt die Angst vorübergehend. Doch genau diese Vermeidung verstärkt die Angststörung.
Beispiele:
nicht mehr U-Bahn fahren;
Einkäufe meiden;
ständig Blutdruck oder Puls messen;
Medikamente „für den Notfall“ mitnehmen und sich ohne sie unsicher fühlen;
Sport meiden aus Angst vor Herzrasen.
Dieses Verhalten bringt kurzfristige Erleichterung, lässt das Gehirn aber nicht lernen, dass die Situation sicher ist.

❤️ Körperliche Reaktionen
Angst geht immer mit körperlichen Veränderungen einher. Sie sind ein normaler Teil der Stressreaktion, können sich aber wie Anzeichen einer gefährlichen Krankheit anfühlen.
Beispiele:
Herzrasen;
Atemnot;
Muskelverspannung;
Zittern;
Schwitzen;
Schwindel;
Kloß im Hals;
Magenbeschwerden;
Kribbeln in Händen oder Gesicht.
Diese Symptome sind unangenehm, sind aber für sich genommen meist nicht gefährlich.

Das Wichtigste
Diese vier Faktoren beeinflussen sich ständig gegenseitig.
Zum Beispiel:
Schlecht geschlafen → das Nervensystem wurde empfindlicher → morgens Herzrasen gespürt → der Gedanke „mit mir stimmt etwas nicht“ → Training abgesagt und zu Hause geblieben → Angst sank einige Stunden, aber das Gehirn „lernte“, dass Vermeidung hilft → beim nächsten Mal tritt Angst noch schneller auf.
Deshalb umfasst das Tagebuch Schlaf, Medikamente, körperliche Aktivität, Auslöser, Tagesereignisse und Angstniveau. Zusammen helfen sie, nicht einzelne Symptome, sondern die ganze Ereigniskette zu sehen. Das macht den Zustand verständlicher und die Behandlung bewusster und wirksamer. Angst ist kein zufälliges Chaos — es ist ein Prozess, den man nach und nach erkennen und verändern lernen kann.`;

const IMPORTANT_INFO_ZH = `焦虑障碍由多个相互关联的因素维持。即使改变其中一个，也能逐渐减轻焦虑。

🧠 神经系统的特点
在焦虑障碍中，神经系统对危险信号变得更加敏感。即使没有真正的威胁，也可能触发焦虑反应。
例如：
睡眠不足；
长期压力；
过度疲劳；
咖啡因或其他兴奋剂；
漏服处方药；
恐慌发作后的恢复期，神经系统仍处于警觉状态。
因此，日记中需要记录：睡眠、用药、身体活动和整体感受。

💭 思维习惯
在焦虑障碍中，大脑会自动寻找危险，并常常高估坏结果发生的可能性。
例如：
「胸口刺痛——一定是心脏问题。」
「如果头晕，我会晕倒。」
「今天有焦虑，明天会更糟。」
「我永远无法正常生活。」
这些想法自动出现，似乎很有说服力，但往往不符合现实。
记录每天的事件和感受，有助于日后更客观地看待情况。

🚶 行为
当一个人开始回避看似危险的情境时，焦虑会暂时减轻。但正是这种回避强化了焦虑障碍。
例如：
不再坐地铁；
不去商店；
不断测量血压或脉搏；
随身携带药物「以防万一」，没有它们就感到不安全；
因害怕心跳加速而避免运动。
这种行为带来短期缓解，但无法让大脑相信情境是安全的。

❤️ 身体的生理反应
焦虑总是伴随身体变化。它们是压力反应的正常部分，但可能被感受为严重疾病的迹象。
例如：
心跳加快；
气短；
肌肉紧张；
颤抖；
出汗；
头晕；
喉咙异物感；
胃部不适；
手或面部发麻。
这些症状令人不适，但通常本身并不危险。

最重要的是
这四个因素不断相互影响。
例如：
睡得不好 → 神经系统更敏感 → 早晨感到心跳加快 → 出现「我出问题了」的想法 → 取消锻炼待在家里 → 焦虑减轻几小时，但大脑「记住」回避有效 → 下次焦虑出现得更快。
因此，日记包含睡眠、药物、身体活动、触发因素、当日事件和焦虑水平。它们一起帮助看到的不是孤立症状，而是完整的事件链。这使状态更易理解，治疗更有意识和更有效。焦虑不是随机混乱——而是一个可以逐渐学会识别和改变的过程。`;

const IMPORTANT_INFO_PT = `O transtorno de ansiedade é mantido por vários fatores interligados. Mudar mesmo um deles ajuda gradualmente a reduzir a ansiedade.

🧠 Como funciona o sistema nervoso
No transtorno de ansiedade, o sistema nervoso fica mais sensível a sinais de perigo. Pode desencadear ansiedade mesmo quando não há ameaça real.
Exemplos:
falta de sono;
estresse prolongado;
exaustão;
cafeína ou outros estimulantes;
esquecer a medicação prescrita;
recuperação após um ataque de pânico, quando o sistema nervoso ainda está em alerta.
Por isso o diário registra sono, medicação, atividade física e bem-estar geral.

💭 Hábitos de pensamento
No transtorno de ansiedade, o cérebro procura perigo automaticamente e muitas vezes superestima a probabilidade de um resultado ruim.
Exemplos:
«Sinto dor no peito — deve ser o coração.»
«Se ficar tonto, vou desmaiar.»
«Se tive ansiedade hoje, amanhã será pior.»
«Nunca vou conseguir viver normalmente.»
Esses pensamentos surgem sozinhos e parecem convincentes, embora muitas vezes não correspondam à realidade.
Anotar eventos do dia e sentimentos ajuda a ver a situação com mais objetividade depois.

🚶 Comportamento
Quando a pessoa começa a evitar situações que parecem perigosas, a ansiedade diminui temporariamente. Mas essa evitação reforça o transtorno de ansiedade.
Exemplos:
parar de usar o metrô;
evitar ir à loja;
medir constantemente pressão ou pulso;
carregar remédios «por precaução» e sentir-se inseguro sem eles;
evitar exercícios por medo de taquicardia.
Esse comportamento traz alívio a curto prazo, mas não permite que o cérebro aprenda que a situação é segura.

❤️ Reações físicas
A ansiedade sempre vem com mudanças no corpo. São parte normal da resposta ao estresse, mas podem parecer sinais de doença grave.
Exemplos:
taquicardia;
falta de ar;
tensão muscular;
tremor;
suor;
tontura;
nó na garganta;
desconforto estomacal;
formigamento nas mãos ou no rosto.
Esses sintomas são desagradáveis, mas geralmente não são perigosos por si só.

O mais importante
Esses quatro fatores influenciam-se constantemente.
Por exemplo:
Dormiu mal → o sistema nervoso ficou mais sensível → de manhã sentiu taquicardia → surgiu o pensamento «algo está errado comigo» → cancelou o treino e ficou em casa → a ansiedade diminuiu por algumas horas, mas o cérebro «aprendeu» que evitar ajuda → da próxima vez a ansiedade aparece ainda mais rápido.
Por isso o diário inclui sono, medicação, atividade física, gatilhos, eventos do dia e nível de ansiedade. Juntos ajudam a ver não sintomas isolados, mas toda a cadeia de eventos. Isso torna o estado mais compreensível e o tratamento mais consciente e eficaz. A ansiedade não é caos aleatório — é um processo que você pode aprender a reconhecer e mudar gradualmente.`;

const IMPORTANT_INFO_IT = `Il disturbo d'ansia è mantenuto da diversi fattori interconnessi. Modificarne anche uno solo aiuta gradualmente a ridurre l'ansia.

🧠 Come funziona il sistema nervoso
Con il disturbo d'ansia, il sistema nervoso diventa più sensibile ai segnali di pericolo. Può scatenare ansia anche quando non c'è una minaccia reale.
Esempi:
mancanza di sonno;
stress prolungato;
esaurimento;
caffeina o altri stimolanti;
saltare un farmaco prescritto;
recupero dopo un attacco di panico, quando il sistema nervoso è ancora in allerta.
Per questo il diario registra sonno, farmaci, attività fisica e benessere generale.

💭 Abitudini di pensiero
Con il disturbo d'ansia, il cervello cerca automaticamente il pericolo e spesso sovrastima la probabilità di un esito negativo.
Esempi:
«Mi fa male il petto — deve essere il cuore.»
«Se mi gira la testa, svengo.»
«Se oggi ho avuto ansia, domani sarà peggio.»
«Non riuscirò mai a vivere normalmente.»
Questi pensieri sorgono da soli e sembrano convincenti, anche se spesso non corrispondono alla realtà.
Annotare eventi della giornata e emozioni aiuta a vedere la situazione in modo più obiettivo in seguito.

🚶 Comportamento
Quando una persona inizia a evitare situazioni che sembrano pericolose, l'ansia diminuisce temporaneamente. Ma proprio questa evitazione rafforza il disturbo d'ansia.
Esempi:
smettere di prendere la metro;
evitare di andare al negozio;
misurare costantemente pressione o polso;
portare farmaci «per ogni evenienza» e sentirsi insicuro senza;
evitare l'esercizio per paura del battito accelerato.
Questo comportamento porta sollievo a breve termine, ma non permette al cervello di imparare che la situazione è sicura.

❤️ Reazioni fisiche
L'ansia è sempre accompagnata da cambiamenti nel corpo. Sono parte normale della risposta allo stress, ma possono sembrare segni di una malattia grave.
Esempi:
battito accelerato;
fiato corto;
tensione muscolare;
tremore;
sudore;
capogiro;
nodo in gola;
disagio gastrico;
formicolio a mani o viso.
Questi sintomi sono sgradevoli, ma di solito non sono pericolosi di per sé.

Soprattutto
Questi quattro fattori si influenzano costantemente a vicenda.
Per esempio:
Hai dormito male → il sistema nervoso è diventato più sensibile → al mattino hai sentito il battito accelerato → è comparso il pensiero «c'è qualcosa che non va» → hai annullato l'allenamento e sei rimasto a casa → l'ansia è diminuita per qualche ora, ma il cervello ha «imparato» che evitare aiuta → la prossima volta l'ansia compare ancora più in fretta.
Per questo il diario include sonno, farmaci, attività fisica, trigger, eventi della giornata e livello d'ansia. Insieme aiutano a vedere non sintomi isolati, ma l'intera catena di eventi. Questo rende lo stato più comprensibile e il trattamento più consapevole ed efficace. L'ansia non è caos casuale — è un processo che puoi imparare a riconoscere e cambiare gradualmente.`;

const IMPORTANT_INFO_JA = `不安障害は、複数の相互に関連する要因によって維持されています。そのうちの一つでも変えていくことで、不安は徐々に軽減していきます。

🧠 神経系の働き
不安障害があると、神経系は危険信号により敏感になります。本当の脅威がない場合でも、不安反応が起こることがあります。
例：
睡眠不足；
長期間のストレス；
燃え尽き；
カフェインなどの興奮剤；
処方薬の飲み忘れ；
パニック発作の後の回復期で、神経系がまだ高い警戒状態にあるとき。
そのため、日記では睡眠、服薬、身体活動、全体的な状態を記録することが大切です。

💭 思考のクセ
不安障害では、脳が自動的に危険を探し、悪い結果の可能性を過大評価することが多くなります。
例：
「胸が痛む——心臓のせいに違いない。」
「めまいがしたら、気を失う。」
「今日不安だったから、明日はもっと悪くなる。」
「もう普通に生きられない。」
これらの考えは自動的に浮かび、説得力があるように感じますが、多くの場合現実とは一致しません。
その日の出来事や気持ちを書き留めることで、後からより客観的に状況を見られるようになります。

🚶 行動
危険だと感じる状況を避け始めると、不安は一時的に減ります。しかし、この回避行動は不安障害を強化します。
例：
地下鉄に乗るのをやめる；
買い物に行くのを避ける；
血圧や脈を常に測る；
「念のため」薬を持ち歩き、それがないと不安になる；
動悸を恐れて運動を避ける。
この行動は短期的な安心感をもたらしますが、状況が安全だと脳が学ぶ機会を与えません。

❤️ 身体反応
不安には必ず身体の変化が伴います。これらはストレス反応の正常な部分ですが、危険な病気の兆候のように感じられることがあります。
例：
動悸；
息苦しさ；
筋肉の緊張；
震え；
発汗；
めまい；
のどの詰まり感；
胃の不快感；
手や顔のしびれ。
これらの症状は不快ですが、それ自体では通常危険ではありません。

最も重要なこと
この4つの要因は常に互いに影響し合います。
例えば：
睡眠不足 → 神経系がより敏感に → 朝、動悸に気づく → 「何かおかしい」という考えが浮かぶ → 運動をキャンセルして家にいる → 数時間不安が減るが、脳は回避が効くと「学習」する → 次回はさらに早く不安が現れる。
そのため、日記には睡眠、服薬、身体活動、トリガー、その日の出来事、不安レベルが含まれます。これらを合わせることで、孤立した症状ではなく出来事の連鎖全体を見られるようになります。状態がより理解しやすくなり、治療がより意識的で効果的になります。不安はランダムな混乱ではなく、徐々に認識し、変えていくことができるプロセスです。`;

const IMPORTANT_INFO_KO = `불안 장애는 여러 상호 연결된 요인에 의해 유지됩니다. 그중 하나만 바꿔도 점차 불안을 줄이는 데 도움이 됩니다.

🧠 신경계의 작동
불안 장애가 있으면 신경계가 위험 신호에 더 민감해집니다. 실제 위협이 없어도 불안 반응이 일어날 수 있습니다.
예:
수면 부족;
장기적인 스트레스;
번아웃;
카페인 등 각성제;
처방약 복용 누락;
공황 발작 후 회복 기간, 신경계가 여전히 경계 상태일 때.
그래서 일기에는 수면, 복약, 신체 활동, 전반적인 컨디션을 기록합니다.

💭 사고 습관
불안 장애에서는 뇌가 자동으로 위험을 찾고, 나쁜 결과의 가능성을 과대평가하는 경우가 많습니다.
예:
「가슴이 아프다 — 심장 문제일 거야.」
「어지러우면 기절할 거야.」
「오늘 불안했으니, 내일은 더 심해질 거야.」
「평범하게 살 수 없을 거야.」
이런 생각은 자동으로 떠오르며 설득력 있게 느껴지지만, 실제와 맞지 않는 경우가 많습니다.
하루의 사건과 감정을 기록해 두면 나중에 더 객관적으로 상황을 볼 수 있습니다.

🚶 행동
위험하다고 느껴지는 상황을 피하기 시작하면 불안이 일시적으로 줄어듭니다. 하지만 이 회피는 불안 장애를 강화합니다.
예:
지하철 이용 중단;
마트 가기 피하기;
혈압이나 맥박을 계속 측정;
「혹시 모르니」약을 들고 다니며 없으면 불안함;
심박 수 증가를 두려워 운동 회피.
이런 행동은 단기적으로는 안도감을 주지만, 상황이 안전하다는 것을 뇌가 배우지 못하게 합니다.

❤️ 신체 반응
불안은 항상 몸의 변화와 함께 합니다. 스트레스 반응의 정상적인 부분이지만, 위험한 질병의 징후처럼 느껴질 수 있습니다.
예:
심박 증가;
숨 가쁨;
근육 긴장;
떨림;
발한;
어지러움;
목에 덩어리 같은 느낌;
위 불편;
손이나 얼굴의 저림.
이 증상들은 불편하지만, 그 자체로는 보통 위험하지 않습니다.

가장 중요한 점
이 네 가지 요인은 끊임없이 서로 영향을 줍니다.
예를 들어:
잠을 못 잤다 → 신경계가 더 민감해짐 → 아침에 심박 증가를 느낌 → 「내 몸에 뭔가 이상하다」는 생각 등장 → 운동을 취소하고 집에 머뭄 → 몇 시간 불안이 줄었지만, 뇌는 회피가 도움이 된다고 「학습」 → 다음에는 불안이 더 빨리 나타남.
그래서 일기에는 수면, 복약, 신체 활동, 유발 요인, 하루의 사건, 불안 수준이 포함됩니다. 함께 보면 고립된 증상이 아니라 사건의 전체 연쇄를 볼 수 있습니다. 상태를 더 이해하기 쉽고, 치료를 더 의식적이고 효과적으로 만듭니다. 불안은 무작위의 혼란이 아니라, 점차 인식하고 바꿀 수 있는 과정입니다.`;

const IMPORTANT_INFO_BY_LANGUAGE: Record<Language, string> = {
  ru: IMPORTANT_INFO_RU,
  en: IMPORTANT_INFO_EN,
  es: IMPORTANT_INFO_ES,
  fr: IMPORTANT_INFO_FR,
  de: IMPORTANT_INFO_DE,
  zh: IMPORTANT_INFO_ZH,
  pt: IMPORTANT_INFO_PT,
  it: IMPORTANT_INFO_IT,
  ja: IMPORTANT_INFO_JA,
  ko: IMPORTANT_INFO_KO,
};

const IMPORTANT_INFO_SUMMARY_TITLES = new Set([
  'И самое главное',
  'Most importantly',
  'Lo más importante',
  "L'essentiel",
  'Das Wichtigste',
  '最重要的是',
  'O mais importante',
  'Soprattutto',
  '最も重要なこと',
  '가장 중요한 점',
]);

export function getImportantInfoText(language: Language): string {
  return `${getMedicalDisclaimerText(language)}\n\n${IMPORTANT_INFO_BY_LANGUAGE[language]}`;
}

export function isImportantInfoSectionTitle(paragraph: string): boolean {
  const trimmed = paragraph.trim();
  return (
    isLegalSectionTitle(trimmed) ||
    /^[🧠💭🚶❤️]/.test(trimmed) ||
    IMPORTANT_INFO_SUMMARY_TITLES.has(trimmed)
  );
}
